...
# --- Kurumsal mimari için modüler yapı ---
class TrendyolScraper:
    ...
    def safe_find_element(self, by, identifier, timeout=15):
        try:
            element = WebDriverWait(self.driver, timeout).until(EC.presence_of_element_located((by, identifier)))
            return element
    ...
    def safe_find_elements(self, by, identifier, timeout=15):
        try:
            element = WebDriverWait(self.driver, timeout).until(EC.presence_of_all_elements_located((by, identifier)))
            return element
        except Exception as e:
            return e
    ...
    def scrape(self):
        ...
        try:
            ...
            print("ÇEREZ KUTUSU BULUNAMADI!")
        for sellerText in sellers:
            ...
            try:
                ...
                try:
                    ...
                    print("MAĞAZA BULUNAMADI -> ",sellerText)
                    continue
            try:
            ...
            try:
                ...
                print(f"{sellerText} mağazası için değerlendirme bulunamadı! Hata: {e}")
                ...
                if not all(pd.isna(x) or x == "Yeterli Veri Yok" or x == 0 for x in sellerDataRows):
                    self.sellerData.loc[len(self.sellerData)] = sellerDataRows
        self.driver.quit()
        self.sellerData.to_excel("Trendyol Satıcı Bilgileri (Detaylı).xlsx",index=False)
...
def main():
    excel_path = r"YOUR EXCEL_FILE_PATH"  # Set your Excel file path here
    scraper = TrendyolScraper(excel_path)
    scraper.scrape()
    style_excel("SellerDataScraper.xlsx")
